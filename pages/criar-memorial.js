...
export default function CriarMemorial() {
  ...
  const [form, setForm] = useState({
    ...
    values_beliefs: '', important_memories: '', family_names: '',
    topics_to_avoid: '',
  })
  ...
  // FIX 1: Evitar memorial duplicado — verifica se já existe antes de criar
  async function salvar() {
    ...
    try {
      // Verifica se já existe memorial com esse nome para esse usuário
      ...
      if (existente) {
        // Memorial já existe — vai direto para o chat sem duplicar
        window.location.href = `/chat/${existente.id}`
        return
      }

      // Cria o memorial
      const { data: memorial, error: e1 } = await supabase
        .from('memorials')
        .insert({
          user_id: user.id,
          name: form.name,
      ...
      // Cria a personalidade
      const { error: e2 } = await supabase
        .from('memorial_personality')
        .insert({
          ...
          family_names: form.family_names ? { contexto: form.family_names } : {},
          topics_to_avoid: form.topics_to_avoid.split(',').map(t => t.trim()).filter(Boolean),
        })

      if (e2) {
        // Se personalidade falhou, remove o memorial para não ficar órfão
      ...
      console.error(err)
      setErro('Erro ao salvar o memorial. Tente novamente.')
    } finally {
      setSalvando(false)
    }
  }
    ...
    <>
      ...
      <style>{`
        *{margin:0;padding:0;box-sizing:border-box}
        ...
        .erro-box{background:#FEF2F2;border:1px solid #FECACA;border-radius:6px;padding:12px 16px;font-size:13px;color:#991B1B;margin-bottom:16px}
      ...
      <main>
        ...
        {etapa === 0 && (
          <>
            ...
            <div className="field"><label>Sua relação com ela/ele *</label><input placeholder="Ex: Minha mãe, Meu pai, Minha avó..." value={form.relationship} onChange={e => set('relationship', e.target.value)} /></div>
            <div className="grid2">
        ...
        {etapa === 1 && (
          <>
            ...
            <div className="field">
              ...
              <textarea placeholder='Ex: "Falava pausadamente, usava muito Deus abençoe, chamava todo mundo de meu filho..."' value={form.way_of_speaking} onChange={e => set('way_of_speaking', e.target.value)} />
              <div className="hint">Descreva o jeito único de falar, expressões, ritmo da fala.</div>
            </div>
            <div className="field">
              <label>Frases que ela/ele mais usava</label>
              <textarea placeholder={"Uma frase por linha:\nDeus é bom o tempo todo!\nVai com calma, meu filho."} style={{minHeight:'90px'}} value={form.key_phrases} onChange={e => set('key_phrases', e.target.value)} />
            </div>
            <div className="field">
        ...
        {etapa === 2 && (
          <>
            ...
            <p className="sub">Cada detalhe torna as conversas mais ricas e verdadeiras.</p>
            <div className="field"><label>Onde nasceu e cresceu?</label><input placeholder="Ex: Interior de Minas Gerais..." value={form.birthplace} onChange={e => set('birthplace', e.target.value)} /></div>
            <div className="field"><label>Profissão</label><input placeholder="Ex: Agricultora, professora, dona de casa..." value={form.profession} onChange={e => set('profession', e.target.value)} /></div>
            <div className="field">
              <label>História de vida</label>
              <textarea style={{minHeight:'130px'}} placeholder="Conte a história dessa pessoa — a infância, os desafios, as conquistas..." value={form.life_story} onChange={e => set('life_story', e.target.value)} />
            </div>
            <div className="field"><label>Hobbies e paixões</label><input placeholder="Separados por vírgula: jardim, culinária, futebol..." value={form.hobbies} onChange={e => set('hobbies', e.target.value)} /></div>
            <div className="field">
              <label>Valores e crenças</label>
              <textarea style={{minHeight:'80px'}} placeholder="Ex: Muito religiosa, católica fervorosa. Colocava a família em primeiro lugar..." value={form.values_beliefs} onChange={e => set('values_beliefs', e.target.value)} />
            </div>
            <div className="btns">
        ...
        {etapa === 3 && (
          <>
            ...
            <p className="sub">Histórias reais e nomes da família tornam a IA muito mais próxima da pessoa real.</p>
            <div className="field">
              <label>Histórias e memórias marcantes</label>
              <textarea style={{minHeight:'140px'}} placeholder="Escreva histórias que ela contava, momentos marcantes..." value={form.important_memories} onChange={e => set('important_memories', e.target.value)} />
              <div className="hint">Quanto mais histórias, mais autêntica será a conversa.</div>
            </div>
            <div className="field">
              <label>Nomes importantes na vida dela/dele</label>
              <textarea style={{minHeight:'80px'}} placeholder="Ex: Filhos: João e Maria. Marido: Carlos. Netos: Pedro e Ana..." value={form.family_names} onChange={e => set('family_names', e.target.value)} />
            </div>
            <div className="field">
              <label>Assuntos a evitar</label>
              <input placeholder="Ex: brigas familiares, dívidas..." value={form.topics_to_avoid} onChange={e => set('topics_to_avoid', e.target.value)} />
              <div className="hint">A IA vai desviar gentilmente desses assuntos.</div>
            </div>
            <div className="btns">
        ...
        {etapa === 4 && (
          <>
            ...
            <h1>Tudo certo?</h1>
            <p className="sub">Revise antes de criar o memorial de <strong>{form.name}</strong>.</p>
